...
export class CommonServiceService {
    ...
    messages: '';
    SERVER_URL = 'https://api.reservas-lab.ml/';
    ...
    update(id, status, list) {
        const filter = list.filter((a) => a.patient_id === id);
        if (filter.length !== 0) {
    ...
    getSpeciality() {
        return this.http.get(this.SERVER_URL + 'especialidades');
    }

    createSpeciality(data) {
        return this.http.post(`${this.SERVER_URL + 'especialidades'}`, data);
    }

    updateSpeciality(data, id) {
        return this.http.put(`${this.SERVER_URL + 'especialidades'}/${id}`, data);
    }

    deleteSpeciality(id) {
        return this.http.delete(`${this.SERVER_URL + 'especialidades'}/${id}`);
    }

    getDoctors() {
        return this.http.get(this.SERVER_URL + 'doctores');
    }

    getDoctorDetails(id) {
        return this.http.get(`${this.SERVER_URL + 'doctores'}/${id}`);
    }
    ...
    createDoctor(data) {
        return this.http.post(`${this.SERVER_URL + 'doctores'}`, data);
    }
