...
export class MapGridComponent implements OnInit {
    ...
    constructor(public Router: Router,
                public reqdoc: ReqDoctorsService) {
    }
    ...
    initilize() {
        ...
        const mapOptions = {
            zoom: 14,
            center: {lat: -34.98552791975121, lng: -71.24280467272861},
            scrollwheel: true,
        ...
        setTimeout(
            () => {this.setMarkers(this.map, this.doctores); } , 5000
        );
    ...
    setMarkers(map, locations) {
        for (var i = 0; i < locations.length; i++) {
            let item = this.doctores[i];
            let latlng = new google.maps.LatLng(item.geo.lat, item.geo.lng);
